...
class ExchangeSimulator:
    ...
    def consume_md(self, marketData_2_exchSim_q):
        while True:
            ...
            if marketData.ticker in list(self.future2stock.values()):
                self.stockdata = marketData
            elif marketData.ticker in list(self.future2stock.keys()):
    ...
    def Trade(self, order,stockdata,futuredata):
        t = time.time()
        exec_list=[]
        if order.ticker in list(self.future2stock.values()):
            market_data = stockdata
        elif order.ticker in list(self.future2stock.keys()):
            market_data = futuredata
        remain_size = order.size
        execution_size = []
        if order.type == 'LO':  # Limit Order
            if order.direction == 'buy':
                execute_price = [market_data.askPrice1, market_data.askPrice2, market_data.askPrice3, market_data.askPrice4, market_data.askPrice5]
                ask_size = [market_data.askSize1, market_data.askSize2, market_data.askSize3, market_data.askSize4, market_data.askSize5]
                for i in range(5):
                ...
                execute_price = [market_data.bidPrice1, market_data.bidPrice2, market_data.bidPrice3, market_data.bidPrice4, market_data.bidPrice5]
                bid_size = [market_data.bidSize1, market_data.bidSize2, market_data.bidSize3, market_data.bidSize4, market_data.bidSize5]
                for i in range(5):
                    if remain_size > 0 and order.price <= execute_price[i]:
        ...
        if remain_size == 0 or (remain_size != 0 and len(execution_size) != 0):
            for i in range(len(execution_size)):
                ex = SingleStockExecution(order.ticker, order.date, order.submissionTime, order.direction, execute_price[i], execution_size[i])
            ...
            ex = SingleStockExecution(order.ticker, order.date, order.submissionTime, order.direction, 0, 0)
            ex.orderID = str(order.orderID)
